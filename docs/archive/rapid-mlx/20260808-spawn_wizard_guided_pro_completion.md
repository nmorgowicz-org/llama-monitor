...
|---|---|---|
| Phase 0 evidence freeze and contract inventory | Complete | Commit `39634a71`; release identity, route/auth inventory, control contract, fixture freeze, and raw G0 receipts recorded under `docs/archive/rapid-mlx/evidence/spawn-wizard-guided-pro/`. |
...
| Phase 3 introspection/effective/provenance truth | Complete (active wizard authority) | Phase 3 inventory at `docs/archive/rapid-mlx/evidence/spawn-wizard-guided-pro/phase-3-inference-inventory.md` now records closure: filename-based family/MTP/parameter-count/architecture/sampling inference is absent from the active wizard path; local, streamed-HF, and Rapid profile introspection carry native context, hybrid/sliding-attention, multimodal, and metadata status/provenance; degraded estimator paths remain explicitly size-only; and Guided option-copy uses DOM cloning instead of `innerHTML`. MTP advisor capability is now metadata-only. The legacy `model_defaults.rs` helpers remain isolated compatibility code; `/api/model-defaults` uses `SamplingCatalog` with typed architecture/profile provenance and never treats those helpers as wizard authority. |
| Phase 4 shared view descriptor relocation | Complete (superseded by Phase 7 Pro) | The Guided drawer owns one canonical llama.cpp or Rapid-MLX advanced wrapper at a time, derives its total from `controlsForView()`, reports a live changed count, preserves labels/listeners/payload controls by moving wrappers rather than cloning inputs, and hides the inactive backend. Rapid Guided layout keeps the backend-native panel and fit rail visible. Fresh release-built G4 receipt `docs/screenshots/artifacts/wizard-llamacpp/spawn-wizard-guided-drawer--receipt.json` and five original-resolution shots verify llama closed/open, Rapid closed/open, engine switch-back, changed count, and current Pro availability. |
...
1. Record `HEAD`, branch, dirty files, release binary identity, and capture-harness command in a
   Phase 0 receipt under `docs/archive/rapid-mlx/evidence/spawn-wizard-guided-pro/`.
...
### Phase 13 final validation milestone (2026-08-09)

Final independent reconciliation is complete. `cargo clippy -- -D warnings`, `cargo test` (2272 passed, 26 ignored), JavaScript validation/lint, `git diff --check`, release build, formatter, and `cargo test --test auth_routing` (39 passed) pass. The isolated release-built Playwright suite passes after updating the intentional JS module baseline from 87 to 89, aligning the Rapid-MLX recommendation assertion with the current shared main layout, and removing the last filename-based MTP advisor fallback. Both wizard screenshot groups were freshly captured sequentially outside the sandbox at realistic viewports; strict capture-manifest validation and its node test pass, and `scripts/check-unused-screenshots.sh` reports no unreferenced screenshots. The manifest validator distinguishes same-basename llama.cpp/Rapid-MLX launch captures by backend and resolves the group-qualified Rapid registration. No Rust/Cargo/platform-gated files changed in this closure, so the Windows target check is not applicable. The plan is complete pending human acceptance of the fresh evidence set.
