...
| 6 | Cross-backend cache policies, recommendations, and teaching | 5 | A6–A9, A21, A23, A31–A37, A41 | 170k | Partly evidenced. Items 8–9 measurement done (`docs/reference/cache-benchmark-results.md`, 3 receipt dirs) and `cache_ram_mib` is plumbed preset→launch→argv with sentinel tests; item 9's positive-recommendation half still needs a multi-branch/slot-pressure run. Items 1–7 (Rapid `hybrid_cache_entries`/`response_cache_entries`, Auto eligibility, shadow-observation contract) and 10–11 not started. |
...
9. Replace unlimited llama host-cache guidance with accepted D18: unified-memory Auto resolves extra host prompt-state caching to `0` for the normal single-user profile while preserving ordinary common-prefix reuse; only evidence-backed regularly interleaved sessions plus proven surplus may produce a confirmed bounded positive cap. Discrete-GPU Auto may use a bounded system-RAM allowance. Treat `-1` as Advanced explicit Unlimited, never Auto, and handle its enabled sentinel correctly.
    **Measurement for items 8–9 is already in hand** (2026-07-27, verified 2026-07-30): `-cram 0` and `-cram 8192` are indistinguishable at 131K/160K/200K on both fork TTFT (within ~85 ms) and marker recall (identical in all twelve context × phase pairs), so D18's Auto=`0` is evidenced for the single-user Apple profile and `cache_ram_mib` is already plumbed preset→launch→argv with sentinel tests at `src/inference/llama_cpp.rs:1155`. What is *not* evidenced is any **positive** cap: the single live slot retained reusable context in both conditions, so no branch was ever displaced and the host cache was never under the pressure it exists to absorb. A bounded positive recommendation still requires the multi-branch/slot-pressure run. See `docs/reference/cache-benchmark-results.md`.
...
The measurement history that motivates this phase — two stacked tooling bugs, the served three-trunk screening matrix, the K=1 SSM clamp, and the `aliases.json` reliability findings — lives in `docs/reference/rapid-mlx-mtp-evidence.md`. Do not restate its numbers here; cite it. Live state and open questions used to live in a separate working-handoff doc. As of 2026-07-30 they are folded into this section and into §12 of the evidence record, and that doc is **deleted** — its last version is in git history at `396644b` if the narrative is ever wanted. There is no third place to look for MTP state. Note that every served figure in the evidence record is `n=1` screening evidence, explicitly not qualification: 6.5a exists to replace it.
...
15. Make qualification results the input to eligibility: a pairing is offered only where a naturally-eligible lane produced a passing paired-metric result. Unqualified pairings may remain reachable behind an explicit research override, labelled as such.
    **Gated on the same upstream boundary as 6.5a, not on items 12–13.** This item's input is a set of passing naturally-eligible results, and no such result can exist while 6.5a is closed — so unlike 12 and 13 it is not merely unstarted work that a future session could pick up. It becomes possible only on a passing requalification-lane run.
