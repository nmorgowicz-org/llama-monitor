...
2. **Engine refactor (not started)** — change `spawn-wizard-ia.js`'s open/closed and editability logic to read `critical`/`view` instead of `tier`/`profile`; update both `spawn-wizard-llama-ia.js` and `spawn-wizard-mlx-ia.js`'s `GROUPS` to the new axes; keep DOM ids/serialization untouched (I1/I2 invariants carry over unchanged). **Scoping note found while starting this milestone:** the engine (`createWizardIA()`) currently builds the *entire* "Advanced" drawer disclosure keyed only on group `tier` — there is no existing DOM-level distinction yet for where `view:'card'` controls should render inline outside the drawer. That inline-card rendering is really Milestone 4's job (decision cards), so Milestone 2 should be scoped narrowly: swap the engine's `tier`/`profile` comparison for `critical` (drawer membership) without yet building card placement, and defer any visual card UI to Milestone 4 as originally planned.
