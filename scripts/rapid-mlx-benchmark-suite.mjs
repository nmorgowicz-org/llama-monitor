...
async function profileOverridesFor(model) {
  const result = spawnSync('rapid-mlx', ['info', model], { encoding: 'utf8', timeout: 30000, maxBuffer: 1024 * 1024 });
...
function manifestFor(model, identity, config, cells, port, utilization, serverModelPath, chatTemplate, profile = {}) {
  const argv = ['rapid-mlx', ...argvFor(serverModelPath ?? model, config, port, utilization, model, profile)];
  return {
    schema_version: 1,
    benchmark: { name: `Rapid-MLX generated ${cells[0].id}`, purpose: 'Generated by rapid-mlx-benchmark-suite.mjs; do not hand-edit.' },
    runtime: { backend: 'rapid-mlx', version: 'record from /metrics', base_url: `http://127.0.0.1:${port}`, health_path: '/health', metrics_path: '/metrics', fresh_server_per_cell: true, exact_argv: argv },
...
if (command === 'plan') {
  const manifests = cells.map((item, index) => ({
    ...
    manifest: manifestFor(options.model, identity, item.configuration, [item], options.port, options.utilization, null, options.chatTemplate ? { source_path: resolve(options.chatTemplate), note: 'materialized as an isolated overlay at run time' } : null, profile),
  }));
  ...
  try {
    ...
    const manifests = cells.map((item, index) => ({
      label: `${String(index).padStart(2, '0')}-${item.id}`,
      manifest: manifestFor(options.model, identity, item.configuration, [item], options.port, options.utilization, template.modelPath, template.metadata, profile),
    }));
