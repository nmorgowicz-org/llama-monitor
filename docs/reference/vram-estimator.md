...
pub struct ModelArch {
    ...
    pub head_dim: u32,          // head_dim = n_embd / n_heads
    ...
    pub n_experts_used: u32,    // Experts activated per token (display only)
    pub expert_fraction: f64,   // Fraction of params that are expert FFNs (default 0.65)

    // Hybrid linear attention (Qwen3.5 / Qwen3.6 / DeltaNet)
    pub n_attn_layers: u32,             // Layers with a KV cache (0 = all layers)
    pub linear_attn_state_bytes: u64,   // Fixed recurrent state size in bytes (context-independent)
...
5. `"gemma-4"` / `"gemma4"` → `gemma4_heuristic(param_b)` then MoE suffix
6. `"qwen3"` → `standard_heuristic(param_b)` with MoE suffix parsing
7. Any name with `"NB-AMB"` MoE suffix → generic heuristic + MoE suffix
8. Fallback → `standard_heuristic(param_b)`
...
3:1 DeltaNet:Attention ratio — exactly 1/4 of layers are standard softmax attention with KV cache. The remaining 3/4 use a fixed-size recurrent state regardless of context length.

| param_b | n_layers | n_attn_layers | n_kv_heads | head_dim | DeltaNet state |
|---------|----------|--------------|------------|----------|----------------|
| ≤ 35 | 64 | 16 | 4 | 256 | 48 × 48 × 128² × 2 B ≈ 720 MB |
| > 35 (davidau expansion) | 96 | 24 | 4 | 256 | 48 × 72 × 128² × 2 B ≈ 1.1 GB |
...
| param_b | n_layers | n_attn_layers | n_kv_heads | n_experts | n_experts_used |
|---------|----------|--------------|------------|-----------|----------------|
| ≤ 80 | 40 | 10 | 2 | 256 | 9 |
| > 80 (122B) | 48 | 12 | 2 | 256 | 9 |
...
| param_b | n_layers | global_layers (n/6) | n_kv_heads (global) | local_kv_heads | n_experts | window |
|---------|----------|---------------------|---------------------|----------------|-----------|--------|
| < 10 (unconfirmed) | 30 | 5 | 4 | 8 | 0 | 512 |
| 10–30 (26B-A4B) | 30 | 5 | 2 | 8 | 128 | 1024 |
| ≥ 30 (31B dense) | 60 | 10 | 4 | 16 | 0 | 1024 |
...
Where `effective_layers` = `n_attn_layers` if hybrid, else `n_layers`.
...
Covers CUDA/Metal context, KV allocator metadata, and compute buffers.
...
overhead        = gpu_overhead_bytes(ubatch_size)
...
```
usable = available_vram × (1 − headroom_fraction)   // default headroom = 5%
kv_budget = usable − fixed_overhead
```
...
3. Binary-search KV quant × context combinations across three scenarios:
   - **Conservative**: q4_0/q4_0, headroom 15%
   - **Standard**: q8_0/q8_0, headroom 10%
   - **Extended**: q4_0/q4_0, headroom 5% (uses more MoE CPU offload)
4. Pick the standard scenario as the recommended result
...
| Issue | Scope | Status |
|-------|-------|--------|
| `mtp_overhead_bytes` uses a 1.5% heuristic | All MTP models | Estimate; actual varies by model architecture |
| Gemma 4 small variants (< 10B) layer/head counts are unconfirmed | Gemma 4 E2B/E4B | Conservative estimate; update when confirmed |
...
## GGUF Metadata Integration (Planned)

The estimator is designed to accept GGUF-derived values as overrides over heuristics. When a GGUF file is introspected via `POST /api/model/introspect`, the following metadata keys map directly to `ModelArch` fields:

| GGUF key | ModelArch field |
|----------|----------------|
| `llama.block_count` | `n_layers` |
| `llama.attention.head_count_kv` | `n_kv_heads` |
| `llama.attention.key_length` | `head_dim` |
| `llama.expert_count` | `n_experts` |
| `llama.expert_used_count` | `n_experts_used` |
| `llama.attention.sliding_window` | `local_attn_window` |
| `llama.context_length` | (used for n_ctx_train warning) |
| `general.architecture` | (used to select family logic, ignores finetune name) |

The introspection result is cached at `~/.config/llama-monitor/model-cache/<sha256>.json` and takes priority over `from_name_and_params()` when available.
...
3. Set `n_attn_layers` if the model is a hybrid (DeltaNet / SSM), `linear_attn_state_bytes` from the architecture doc
4. Set `n_global_attn_layers` + `local_attn_window` + `local_kv_heads` + `global_head_dim` if sliding-window
5. Set `expert_fraction` from the published parameter breakdown if available; otherwise leave at 0.65
6. Add a unit test with exact arithmetic for at least one known context size
7. Update `docs/reference/vram-estimator.md` (this file) with the new family's heuristic table row
