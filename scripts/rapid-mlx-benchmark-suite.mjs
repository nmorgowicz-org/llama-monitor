...
function installedRapidMlxVersion() {
  const result = spawnSync('rapid-mlx', ['--version'], { encoding: 'utf8', timeout: 30000 });
...
function manifestFor(model, identity, runtimeVersion, config, cells, port, utilization, serverModelPath, chatTemplate, profile = {}) {
...
if (command === 'plan') {
  const manifests = cells.map((item, index) => ({
    ...
    manifest: manifestFor(options.model, identity, runtimeVersion, item.configuration, [item], options.port, options.utilization, null, options.chatTemplate ? { source_path: resolve(options.chatTemplate), note: 'materialized as an isolated overlay at run time' } : null, profile),
  }));
  ...
  try {
    ...
    const manifests = cells.map((item, index) => ({
      label: `${String(index).padStart(2, '0')}-${item.id}`,
      manifest: manifestFor(options.model, identity, runtimeVersion, item.configuration, [item], options.port, options.utilization, template.modelPath, template.metadata, profile),
    }));
