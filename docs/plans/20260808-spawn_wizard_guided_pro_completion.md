...
| Phase 3 introspection/effective/provenance truth | In progress | Phase 3A inventory recorded at `docs/plans/evidence/spawn-wizard-guided-pro/phase-3-inference-inventory.md`; filename-based MTP enabling/default fallbacks are removed in the wizard, and the Guided option-copy path now uses DOM cloning instead of `innerHTML`. Typed local/HF GGUF and MLX provenance/default response work remains. |
