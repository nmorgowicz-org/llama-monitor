...
| `preset-editor-lifecycle-modal-dark.png` | Dark-theme legibility comparable to the rest of the app (e.g. `preset-editor-advanced-tab.png`, which renders at full contrast) | "Version history" / "Community fixes" list-item text renders at ~2.2:1 contrast against the panel background (`--color-text-muted` on `--pe-panel-bg` dark gradient), well under WCAG's 3:1 floor for muted/secondary text. Confirmed via direct pixel sampling (panel bg ≈ rgb(28,34,41), text ≈ rgb(70-95,75-100,80-105)), not a screenshot rendering artifact. | **Medium-high** — real accessibility/legibility defect, dark theme only | **Fixed** — `static/css/modal-premium.css` (~line 3068-3079) overrides `.chat-template-lifecycle-section-content` / `.chat-template-lifecycle-section-subtitle` / `.chat-template-lifecycle-subtitle` / `.chat-template-lifecycle-version-label` to `#d3d7de` in `:root:not([data-theme="light"])` scope. Computed contrast vs the panel bg (`#1c2229`) is ~11.1:1, well above the 3:1 floor. |
...
- ~~One open finding: `preset-editor-lifecycle-modal-dark.png`~~ — **fixed**. Dark-theme list-item text in `.chat-template-lifecycle-section-content` now renders at ~11.1:1 contrast (was ~2.2:1) via `modal-premium.css` (~line 3068-3079).
