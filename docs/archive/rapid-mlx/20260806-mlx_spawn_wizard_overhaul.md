...
**Rapid-MLX/MTPLX side is unresearched — flagged as a new Phase-4 sub-task, not assumed identical.** Whether `spawn-rapid-max-num-seqs > 1` has an equivalent effect on Rapid-MLX's speculative-decoding companions group (§2.8, `companions`) is unknown; the archive was not checked for this specific interaction during this planning pass. Phase 4, when building the tier table and the `effective`/interaction-warning wiring, must explicitly test and document Rapid-MLX's behavior here rather than porting the llama.cpp warning by assumption.
