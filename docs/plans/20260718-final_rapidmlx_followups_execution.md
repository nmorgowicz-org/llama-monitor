...
- **Milestone (2026-08-10, MTP evidence contract):** Updated the Spawn Wizard and Preset Advisor `/api/advise` payload to separate confirmed `has_mtp` from provisional `mtp_inferred`. Draft/head hints now remain useful when their files expose no head metadata, while advisory copy tells users to verify compatibility; confirmed introspection remains authoritative. Commit `fix(wizard): label inferred MTP hints`; release build, clippy, focused performance-hint tests, JavaScript validation/lint, and diff checks pass.
- **Milestone (2026-08-10, template qualification convergence):** Reconciled the historical 9b note with current source: the discussion create-fix flow already calls `POST /api/chat-template/smoke-test` and only activates a candidate release after a pass; retained release-history activation is documented as rollback over an installed release. API reference and UI contract both describe the bounded two-fixture gate.
