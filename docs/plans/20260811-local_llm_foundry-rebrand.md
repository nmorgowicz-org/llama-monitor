...
| 11.5 — bounded llama.cpp Calibration v1 | In progress — Phase 2 bounded Quick | Rapid-MLX benchmark output is informational-only and cannot emit llama.cpp tuning patches. Typed contracts, active-root persistence, durable recovery, authenticated preflight/start/poll/cancel, deterministic typed candidate planning, and sequential bounded Quick execution with per-candidate receipts and measured winner selection now pass focused/full Rust validation, release build, clippy, and diff checks. Upstream pin/evidence, full runtime capability probing, Balanced search, transactional apply, UI, and Phase 10 gates remain open. |
