...
- **Phase 5a Parts:**
  - **5a-Part 1 (~40k):** Verified complete — Rapid execution policy types + MemoryBreakdown foundation — items 1-2; new execution_policy.rs with KvCacheDtype {bf16,int8,int4}, TurboQuantMode {v4,k8v4,none}, RapidMlxExecutionPolicy (reasoning→int8 override), MemoryBreakdown (8 additive components); 19 tests pass including no_llama_kv_vocabulary_used; committed 3bba01c
