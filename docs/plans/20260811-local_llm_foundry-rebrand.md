...
| 11.5 — bounded llama.cpp Calibration v1 | In progress — Phase 1/2 boundary | Rapid-MLX benchmark output is informational-only and cannot emit llama.cpp tuning patches. Typed fingerprints, workload/request contracts, active-root receipt paths, durable journal/snapshot primitives, an authenticated llama.cpp-only Quick single-trial executor/API, and deterministic capability-gated typed candidate planning now pass the full Rust suite, release build, focused tests, clippy, and diff checks. Upstream pin/evidence, full runtime capability probing, multi-candidate execution, transactional apply, UI, and Phase 10 gates remain open. |
