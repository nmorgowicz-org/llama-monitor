...
| Qwen3.5/3.6 vision | Historical 0.11.1 qualification found the hybrid `ArraysCache` lane blocked target Qwen models. Upstream Rapid-MLX PR #1798 now provides a serialized hybrid MLLM lane; Qwen3.6-27B and Nightmedia Qwen3.6-35B image requests are live-qualified on upstream `main` with the `vision` extra. | Integrated `image_url` path, Qwen3-VL tower, embedding splice, MTP history integration, and vision cache. | Target VLM support exists; MTP source still has an image-embedding limitation, so draft efficiency/state near images needs direct validation. |
