...
| `src/llama/model_defaults.rs` | Legacy compatibility helpers still contain name-based branches; production `/api/model-defaults` uses `SamplingCatalog` instead | Isolated legacy surface | Keep unreachable compatibility API out of wizard authority; future cleanup should delete or make it metadata-only |
