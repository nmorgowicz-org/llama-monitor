...
**Date:** 2026-06-11
**Status:** Complete (All actionable items done except module split)
...
- Quant table completeness tests (all 60+ entries exist, have valid multipliers)
...
### 6. Split `vram_estimator.rs` into smaller modules — ⏭️ DEFERRED

**File:** `src/llama/vram_estimator.rs` (~2,723 lines)

**Decision:** 🟡 Medium priority concern from review, not a blocker. Splitting a 2,723-line file carries risk of introducing breakage in estimation logic. Deferring this to a separate, focused PR where the refactoring can be reviewed on its own.
...
| 5 | 🟡 | VRAM estimator tests | ⏳ PENDING |
| 6 | 🟡 | Split vram_estimator.rs | ⏳ PENDING |
| 7 | 🟡 | ModelTags warning | ✅ DONE |
| 8 | 🟢 | SSRF DNS rebinding TODO | ✅ DONE |
| 9 | 🟢 | Box<dyn Reply> cleanup | ⏳ PENDING |
| 10 | 🟢 | is_noise_log docs | ⏳ PENDING |
