...
use crate::hf::{fetch_raw_bytes_at, hf_get_model_info, hf_load_token, list_repo_siblings};
use crate::models::community_source_catalog::{entries_for_username, load_catalog};
use std::path::Path;
use std::sync::{LazyLock, Mutex};

static CATALOG_CACHE: LazyLock<
    Mutex<Option<crate::models::community_source_catalog::CommunitySourceCatalog>>,
...
pub async fn hf_qualify_repo(req: QualifyRequest) -> Result<HfQualification, String> {
    ...
    let backend_qualified =
        determine_qualification(&format, &target_backend, config.as_ref(), &siblings);
    let qualification_reason = build_qualification_reason(
        &format,
        &target_backend,
        &backend_hint,
        &backend_qualified,
...
fn detect_format(siblings: &[String]) -> String {
    ...
    let has_pickle = siblings.iter().any(|p| {
        p.to_ascii_lowercase().ends_with(".bin") || p.to_ascii_lowercase().ends_with(".pt")
    });
...
async fn collect_config_from_json(repo_id: &str, revision: &str) -> Option<HfConfigEvidence> {
    ...
    if let Ok(cfg) =
        crate::hf::fetch_mlx_config_revision_aware(repo_id, revision, "model.safetensors").await
    {
        let name_lower = repo_id.to_ascii_lowercase();
        let model_type = cfg
            .model_type
            .clone()
            .unwrap_or_else(|| infer_model_type_from_name(&name_lower));
...
async fn collect_weight_evidence(
    ...
    let weight_files: Vec<&String> = siblings
        .iter()
        .filter(|p| {
            let l = p.to_ascii_lowercase();
...
async fn fetch_weight_total_bytes(repo_id: &str, _siblings: &[String]) -> Result<u64, String> {
    ...
    let items: Vec<serde_json::Value> =
        resp.json().await.map_err(|e| format!("tree parse: {e}"))?;
...
async fn collect_extras_evidence(
    ...
    let name_lower = repo_id.to_ascii_lowercase();
    let tags_lower: Vec<String> = runtime
        .tags
        .iter()
    ...
    let tool_use = tags_lower
        .iter()
        .any(|t| t.contains("tool_use") || t.contains("tools"))
    ...
    let reasoning = tags_lower
        .iter()
        .any(|t| t.contains("reasoning") || t.contains("cot"))
...
async fn fetch_readme_text(repo_id: &str) -> Result<String, String> {
    ...
    let resp = req
        .send()
        .await
        .map_err(|e| format!("readme request: {e}"))?;
    if !resp.status().is_success() {
    ...
    let bytes = resp
        .bytes()
        .await
        .map_err(|e| format!("readme read: {e}"))?;
    ...
    match fetch_readme_text(repo_id).await {
        Ok(readme) => {
            for line in readme.lines().take(50) {
                ...
                if (ll.contains("model_name:") || ll.contains("model_family:"))
                    && line.contains(':')
                {
...
fn determine_backend_hint(
    format: &str,
    config: Option<&HfConfigEvidence>,
    _extras: Option<&HfExtrasEvidence>,
) -> String {
    ...
    let mlx_native =
        name_lower.contains("mlx") || name_lower.contains("mamba") || name_lower.contains("ssm");
...
fn build_qualification_reason(
    ...
    if extra_hints.is_empty() {
        format!(
            "Qualified for {}: format '{}', architecture '{}' — suggested backend: {}",
            target, format, model_type, backend_hint
        )
    } else {
        format!(
            "Qualified for {}: format '{}', architecture '{}' (hints: {}) — suggested backend: {}",
...
fn infer_model_type_from_name(name: &str) -> String {
    ...
    if name_lower.contains("qwen3.6") || name_lower.contains("qwen36") {
        "qwen3.6".into()
    } else if name_lower.contains("qwen3.5") || name_lower.contains("qwen35") {
        "qwen3.5".into()
    } else if name_lower.contains("qwen3") {
        "qwen3".into()
    } else if name_lower.contains("qwen") {
        "qwen".into()
    } else if name_lower.contains("llama-3.3") || name_lower.contains("llama33") {
        "llama3.3".into()
...
pub async fn hf_resolve_identity(
    ...
    Ok(HfIdentity {
        ...
        revision,
        original_author,
        base_model: if base_models.is_empty() {
            None
        } else {
            Some(base_models)
        },
...
fn resolve_identity_roles(
    ...
    if is_gguf || is_mlx {
        ...
        converter_role = Some(HfIdentityConverter {
            ...
            confidence: if catalog_entry.is_some() {
                "high".into()
            } else {
                "medium".into()
            },
    ...
    if original_author.is_none() && !base_models.is_empty() {
        if let Some(entry) = entries_for_username(catalog, repo_owner).first() {
            if matches!(
                entry.role,
                crate::models::community_source_catalog::CommunitySourceRole::OriginalAuthor
            ) {
    ...
    let has_ft_name = name_lower.contains("ft-")
        || name_lower.contains("-ft")
        || name_lower.contains("finetune")
        || name_lower.contains("fine-tune");
    let has_ft_tag = tags_lower
        .iter()
        .any(|t| t.contains("fine-tune") || t.contains("finetune") || t.contains("ft:"));
    let has_ft_readme = readme.to_ascii_lowercase().contains("fine-tune")
        || readme.to_ascii_lowercase().contains("finetune");
    ...
    let has_merge_name = name_lower.contains("merge")
        || name_lower.contains("distill")
        || name_lower.contains("heretic")
        || name_lower.contains("abliterated");
    let has_merge_tag = tags_lower
        .iter()
        .any(|t| t.contains("merge") || t.contains("distill"));
...
fn has_first_party_evidence(tags: &[String], siblings: &[String]) -> bool {
    let has_safetensors = siblings
        .iter()
        .any(|p| p.to_ascii_lowercase().ends_with(".safetensors"));
...
fn detect_community_author(
    ...
    let has_creation_evidence = tags_lower
        .iter()
        .any(|t| t.contains("fine-tune") || t.contains("merge") || t.contains("distill"))
        || name_lower.contains("ft-")
        || name_lower.contains("heretic");
    ...
    for line in readme.lines().take(100) {
        ...
        if ll.contains("dataset") || ll.contains("training data") {
                ...
                line.split(|c: char| !c.is_alphanumeric() && !matches!(c, '/' | '_' | '-' | '.'))
            {
                if segment.len() > 4
                    && segment.contains('/')
                    && !datasets.contains(&segment.trim().to_string())
                {
...
fn determine_resolution_confidence(
    ...
    let author_confidence = original_author
        .as_ref()
        .map(|a| a.confidence.as_str())
        .unwrap_or("");
