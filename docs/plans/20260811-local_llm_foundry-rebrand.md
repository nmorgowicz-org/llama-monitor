...
| 11.5 — bounded llama.cpp Calibration v1 | In progress — Phase 5 UI boundary | Rapid-MLX benchmark output is informational-only and cannot emit llama.cpp tuning patches. Typed contracts, active-root persistence, durable recovery, authenticated preflight/start/poll/cancel, deterministic typed candidate planning, sequential bounded Quick execution with per-candidate receipts/winner selection, db-admin transactional apply with optimistic conflict checks and derived-preset default, and the Preset Editor Calibrate flow now pass full Rust/JS validation, release build, focused release-built Playwright testing, and an opt-in real-local-GGUF capture whose preflight reaches the real backend while benchmark responses remain intercepted. The remaining Phase 5 gates are full result-review/apply-conflict/rollback coverage, screenshot human acceptance, and post-apply validation; upstream pin/evidence, full runtime capability probing, Balanced search, Phase 6, and Phase 10 gates remain open. |
