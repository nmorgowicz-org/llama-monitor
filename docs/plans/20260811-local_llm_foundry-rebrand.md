...
| 11.5 — bounded llama.cpp Calibration v1 | In progress — Phase 1 single-trial boundary | Rapid-MLX benchmark output is informational-only and cannot emit llama.cpp tuning patches. Typed fingerprints, workload/request contracts, active-root receipt paths, durable journal/snapshot primitives, and an authenticated llama.cpp-only Quick single-trial executor/API now pass the full Rust suite, release build, focused tests, clippy, and diff checks. Upstream pin/evidence, full introspection/capability qualification, Quick/Balanced search, transactional apply, UI, and Phase 10 gates remain open. |
