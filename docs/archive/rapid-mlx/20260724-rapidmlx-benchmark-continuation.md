...
- [`20260718-final_rapidmlx_followups.md`](./20260718-final_rapidmlx_followups.md) for historical product requirements and future loader abstraction.
- [`20260718-final_rapidmlx_followups_execution.md`](./20260718-final_rapidmlx_followups_execution.md) for historical phase routing.
...
The same source envelope on Unsloth Qwen 3.6 35B was also 5/5 throughout: BF16/int8/int4 peaks at 32k were `22.56/22.31/22.13` GB, at 65k `23.79/23.26/22.92`, and at 131k `26.43/25.31/24.64`; int8/int4 were `26.21/25.30` at 160k and `27.46/26.33` at 200k. The 35B runtime was allowed to resolve its documented hybrid architecture itself—no `--force-hybrid` or `--no-hybrid` was passed.

**Architecture/template profile handoff (2026-07-26):** before admitting a community Qwen row, merge revision-pinned MLX `config.json`/nested `text_config` and weight metadata with `rapid-mlx info`; config is authoritative for attention topology, while `info` supplies parser/reasoning and runtime capability hints. Surface a conflict instead of silently choosing one: `nightmedia/Qwen3.6-27B-Architect-Polaris2-Fable-B-F451-Tess-mxfp4-mlx` revision `721c71607072ecc0f0904db862d64ea1d0ac59fb` declares 64 text layers with `full_attention_interval: 4`, 48 `linear_attention` plus 16 `full_attention` layers, and GatedDeltaNet/Mamba fields, while current source `info` says pure attention. Its explicit `--force-hybrid` smoke and full 512-step context qualification both passed 5/5: BF16/int8/int4 peaks were `18.63/18.02/17.75` GB at 32k, `22.77/20.95/19.85` at 65k, and `31.76/27.98/25.68` at 131k; int8/int4 were `30.96/28.16` at 160k and `35.12/31.61` at 200k. These rows are evidence for the forced-hybrid envelope only, never for the current source pure-attention profile. The same resolved profile must populate the Native/qualified-release/custom template choice defined in the execution plan's Phase 9: native template is the default and must re-link original HF-snapshot/local files; Qwen3.5/Qwen3.6 overrides are immutable, provenance-pinned, independently qualified releases rather than mutable `main` downloads.
...
The verified 4096 workaround is safe for the crash envelope, but the standing benchmark/default decision is **`--prefill-step-size 512`**. It has the lowest measured peak allocation and prevents transient prefill buffers from contaminating KV/RAM estimates; 2048/4096 remain explicit throughput experiments only.

### Still open / next

1. **Benchmark harness fix: resolved.** The recurring context and calibration lanes pin `--prefill-step-size 512`; 2048/4096 are available only through the explicit prefill/ubatch comparison suites.
...
3. **`prefill-step-size` UX/hints review (frontend + backend).** The benchmark default is 512, but the param is **not surfaced in `static/js` at all**, and the Rust runtime config exposes `prefill_batch_size` (in `rapid_mlx_runtime.rs`), not `prefill_step_size` — confirm whether these are the same serve flag or two different levers. Add proper user-facing hints: what it does (per-chunk prompt width; drives the O(prefill_step × context × heads) score buffer), when to lower it (high context on Apple Silicon to stay under `max_buffer_length`), the tradeoff (slower prefill), and a suggested formula/auto-clamp.

4. **Re-collect source 32k/65k/131k/160k/200k with the 512 default** before using the measurements for estimator calibration; retain 2048/4096 rows as separate throughput/peak evidence.
