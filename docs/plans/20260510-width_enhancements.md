# Width Enhancements Plan

**Date:** 2026-05-10
**Status:** Planning
**Priority:** High
**Scope:** Chat toolbar responsiveness, shell width model, left sidebar resizing
...
This should not be fixed as a one-off Persona patch. The correct solution is a broader width architecture update that:

1. Measures actual available shell width.
2. Applies progressive density classes to the chat toolbar before mobile.
3. Reprioritizes low-value controls in constrained widths.
4. Consolidates redundant file actions.
5. Adds a resizable main left sidebar using the same general interaction model as the existing chat input resize handle.

The long-term trigger for responsive density should be a `ResizeObserver` on the rendered content area or chat header, not viewport width guesses alone.
...
There is also persisted sidebar-width-related state elsewhere:
...
- chat notes sidebar width persistence in [static/js/features/chat-notes.js](/Users/nick/SCRIPTS/CLAUDE/llama-monitor/static/js/features/chat-notes.js:58)

At the time of writing, that persisted width plumbing does not appear to control the main left navigation shell.
...
- light theme rules still cover new states
...
When implementation begins, the agent should start by introducing the shell width model and density classes first. Do not begin with a Persona-only CSS patch. The correct sequence is:
...
That sequence fixes the root cause and leaves the shell in a maintainable state for future UI work.
