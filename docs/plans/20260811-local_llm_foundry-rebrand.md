...
| 11.5 — bounded llama.cpp Calibration v1 | In progress — Phase 5 backend safety boundary | Rapid-MLX benchmark output is informational-only and cannot emit llama.cpp tuning patches. Typed contracts, active-root persistence, durable recovery, authenticated preflight/start/poll/cancel, deterministic typed candidate planning, sequential bounded Quick execution with per-candidate receipts/winner selection, db-admin transactional apply with optimistic conflict checks and derived-preset default, and the Preset Editor Calibrate flow now pass full Rust/JS validation, release build, five focused release-built Playwright cases, and an opt-in real-local-GGUF capture whose preflight reaches the real backend while benchmark responses remain intercepted. The UI-side stale/cancel/confirmation/conflict/Rapid gates are covered; durable rollback and short post-apply validation remain backend implementation gates, with screenshot human acceptance still open. Upstream pin/evidence, full runtime capability probing, Balanced search, Phase 6, and Phase 10 gates remain open. |
