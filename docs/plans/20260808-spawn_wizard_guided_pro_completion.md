...
| Phase 3 introspection/effective/provenance truth | In progress | Phase 3A inventory recorded at `docs/plans/evidence/spawn-wizard-guided-pro/phase-3-inference-inventory.md`; filename-based family/MTP/architecture fallbacks are removed from the active wizard path, local and streamed-HF introspection now merge native context, hybrid/sliding-attention, multimodal, and metadata status/provenance fields, degraded GGUF/HF/MLX estimator paths are size-only rather than name-derived, and the Guided option-copy path uses DOM cloning instead of `innerHTML`. `cargo clippy -- -D warnings`, `cargo test` (2270 passed, 26 ignored), JS validation/lint, strict capture-manifest/receipt tests, and release build pass. Fresh release-built tier evidence plus `spawn-wizard-mmproj-selection--receipt.json` / original-resolution `spawn-wizard-mmproj-selection--llamacpp-local--vision.png` verify the hardware vision selector and typed family-backed F16 recommendation. MLX source/config provenance UI and the remaining backend model-defaults fixture coverage remain. |
