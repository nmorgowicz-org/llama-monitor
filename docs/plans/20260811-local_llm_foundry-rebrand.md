...
| 11.5 — bounded llama.cpp Calibration v1 | In progress — Phase 0 | Rapid-MLX benchmark output is now informational-only and cannot emit llama.cpp tuning patches (`classify_rapid_mlx_benchmark_result`; focused Rust test passes). Upstream contracts, introspection-only qualification, and the remaining Phase 0 evidence are still open. |
