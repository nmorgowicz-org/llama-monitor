...
| 11.5 — bounded llama.cpp Calibration v1 | In progress — Phase 0/1 foundation | Rapid-MLX benchmark output is informational-only and cannot emit llama.cpp tuning patches; typed Calibration fingerprints, workload/request contracts, active-root receipt paths, and durable journal/snapshot primitives now pass focused Rust tests and clippy. Upstream contracts, introspection-only qualification, executor/API, and the remaining Phase 0 evidence are still open. |
