...
**Note on multi-client enrollment:** Automatic SSH bootstrap enrollment is implemented and shipped (see §20). Android connects to RYNE with zero manual steps beyond SSH setup — no manual CA deployment, no restart required.
...
| **Android client cert not trusted by RYNE** | ~~High~~ **Resolved** | **SSH bootstrap enrollment implemented (§20) — automatic on first connection, no manual steps** |
...
## 20. Multi-Client Remote Agent Enrollment

**Added:** 2026-05-25  
**Updated:** 2026-05-25 — fully implemented via SSH bootstrap  
**Priority:** ~~Phase 4~~ **Done** — shipped in `fix/multi-client-agent-enrollment`  
**Status:** Resolved — automatic SSH bootstrap enrollment is live

### 20.1 What Was Built

The original gap (no self-service enrollment API, manual `adb`/`scp` workaround required) is fully resolved. The implementation landed across `src/agent.rs` and `src/certs.rs`:

**Per-device CA model:**
```
~/.config/llama-monitor/certs/
├── ca.pem                     # This device's CA
├── ca.key                     # This device's CA key (never leaves device)
├── agent-client.pem           # This device's client cert (signed by ca.pem)
├── agent-client.key
├── cas/
│   ├── <instance-id-1>.pem   # Enrolled client CA from device 1 (agent-side)
│   └── <instance-id-2>.pem   # Enrolled client CA from device 2, etc.
└── remote-cas/
    └── ryne.pem               # Agent's CA cert (fetched via SSH, client-side)
```

**Dynamic CA reload:** The agent uses a `tokio::sync::watch` channel to hot-reload `cas/` entries — new devices can connect without restarting the agent.

**SSH bootstrap enrollment (automatic):** When a dashboard fails to connect to a remote agent (mTLS rejection) and SSH is configured, the poller automatically:

1. SSHes to the remote host and reads the agent's `ca.pem` → saves to `remote-cas/<hostname>.pem`
2. Reads the api-token from the remote machine via SSH
3. Ensures this device has a local CA + client cert (generates if missing)
4. POSTs the device's CA to the agent's enrollment endpoint (`Authorization: Bearer <api-token>`)
5. Saves the api-token to settings for ongoing API calls

On the next poll iteration, mTLS succeeds in both directions. No user interaction is required beyond SSH setup (which is already required for autostart).

**Enrollment endpoint:**
```
POST /enroll   (separate port: agent_port + 1, server-only TLS, api-token auth)
...
### 20.2 Android Behavior

On first connection attempt from Android, the poller detects the mTLS failure and triggers SSH bootstrap automatically. The user only sets the llama-server endpoint and SSH target — no codes, QR codes, or manual file transfers.

A fresh Android setup (no prior Mac connection) works end-to-end: Android generates its own CA, bootstraps enrollment via SSH, and connects. A Mac connecting afterward gets its own independent `cas/` entry — existing Android enrollment is unaffected.

### 20.3 Files Changed

```
src/agent.rs        — bootstrap_client_enrollment, read_remote_ca_pem,
                      poller enrollment logic, build_agent_https_client,
                      build_enrollment_https_client, run_enrollment_server,
                      handle_register_ca
src/certs.rs        — remote_agent_cas_dir, save_remote_agent_ca,
                      load_remote_agent_ca_certs
docs/reference/     — remote-agent.md §mTLS, tls-architecture.md §mTLS
```
