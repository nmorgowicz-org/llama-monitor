...
- **Phase 5a Parts:**
  - **5a-Part 1 (~40k):** Rapid execution policy types + MemoryBreakdown foundation — items 1-2; files: new execution_policy.rs, llama/vram_estimator/estimate.rs (new types), rapid_mlx/command.rs; deliver: RapidMlxExecutionPolicy, MemoryBreakdown with Rapid-native KV vocabulary (bf16/int8/int4), requested/effective distinction, reason strings; hard gates: no llama ctk/ctv; requested/effective distinct
  - **5a-Part 2 (~45k):** TurboQuant/D31 + active vs retained separation — items 3-5; files: execution_policy.rs, estimate.rs (retained cache math), web/api/vram.rs; deliver: D31 TurboQuant (Auto/Standard/K8V4/V-only), exact-alias resolution, active/retained split, transient decompression peak; hard gates: TurboQuant only on qualified retained; active/retained distinct; Standard not FP16
